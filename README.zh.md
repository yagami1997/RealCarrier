...
<p>
  <i>轻量级高效的美国电话号码运营商查询工具，支持中英文双语界面及快速批量查询</i>
</p>
...
| **虚拟号码** | 通过VoIP服务提供的号码，不依赖特定物理位置或设备，由虚拟运营商（Twilio、Bandwidth、Telnyx等）提供 |

区分这两种类型对于识别潜在的欺诈活动、验证用户身份和确保通信合规性至关重要。
</details>

<details>
<summary>🌐 API供应商</summary>
...
**特别提示：**注册Telnyx时使用的IP地址和付款方式（信用卡/PayPal）**必须来自同一个国家**。例如，如果您使用新加坡IP注册但用美国PayPal账户付款，您的注册将被拒绝。但如果您使用美国IP注册并使用美国PayPal账户，则不会被拒绝。
...
<details>
<summary>📖 Telnyx号码查询API开发者指南</summary>

## 📱 Telnyx Number Lookup API 开发者指南

本指南为开发者提供将Telnyx Number Lookup API集成到RealCarrier项目的必要信息。
...
2. **获取API密钥**
   - 在Telnyx门户网站的API密钥部分导航
   - 为您的应用程序生成新的API密钥
   - 安全存储此密钥，因为所有API请求都需要它

3. **IP兼容性注意事项**
   - 确保注册时使用的IP地址与您的支付方式所在国家匹配

### 📋 理解Number Lookup API

Number Lookup API提供美国电话号码的详细信息，特别强调：
...
2. **运营商部分**
   ...
   - `type`：线路类型（mobile、landline、voip）
...
3. **虚拟号码检测**
   - 查找`carrier.type`值为"voip"
   - 检查运营商名称与虚拟提供商数据库的匹配情况
</details>
...
- 本工具使用Telnyx和Twilio API，可能会产生API调用费用
- **重要要求**：
  - **Telnyx**：需要完成KYC和充值才能正常使用
  - **Twilio**：需要完成身份验证和账户充值才能使用API服务
- 使用前请确保您的API账户状态正常，以避免查询失败
...
---
<div align="center">
<b>最后更新</b>：2025-04-09 15:30:42 (太平洋时间)
</div>
